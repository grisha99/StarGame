...
public class Rect {
    ...
    public final Vector2 pos = new Vector2();   // координаты по центру объектп
    ...
    public void set(Rect from) {
        pos.set(from.pos);
        halfWidth = from.getHalfWidth();
        halfHeight = from.getHalfHeight();
    }
    ...
    public boolean isMe(Vector2 touch) {
        return touch.x >= getLeft() && touch.x <= getRight() && touch.y >= getBottom() &&
               touch.y <= getTop();
    }
